...
\begin{algorithm}
	\caption{Hamiltonian Monte Carlo MCMC}
	\begin{algorithmic}[1]
	\Procedure{HMC}{$U$, $\nabla U$, $\epsilon$, $L$, $q_{current}$}
		\State $ q \gets q_{current}$
		\State $ p \sim \mathcal{N}(len(q),0,1)$
		\State $ p_{current} \gets p$
		\State $ p \gets p - \frac{\epsilon}{2}\nabla U(q)$ \Comment{make half step for momentum} 
		\For{$i \text{ in } 1:L$}
		\State $q \gets q + epsilon * p$ \Comment{make full step for the position} 
		\If{$i != L$}
		\State $p \gets p- \epsilon \nabla U(q)$
		\EndIf
		\EndFor
		\State $p - \frac{\epsilon}{2}\nabla U(q)$ \Comment{Half step for momentum}
		...
		\State $q \gets q$ \Comment{Accept}
		\Else
		\State $q_{current} \gets q_{current}$ \Comment{Reject}
		\EndIf
	\end{algorithmic} 
\end{algorithm}
\end{document} 